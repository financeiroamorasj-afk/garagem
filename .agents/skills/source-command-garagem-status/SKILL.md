---
name: "source-command-garagem-status"
...
# source-command-garagem-status

Use this skill when the user asks to run the migrated source command `garagem-status`.
...
Este comando **não implementa nada**. É leitura e relatório, para eu (Rafa) e o Codex (chat) decidirmos juntos qual frente entra primeiro: Módulo Financeiro, Captação de Lead, ou padrões gerais do adm-repi.
