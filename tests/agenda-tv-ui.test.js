...
test('modo TV é administrativo, somente leitura, responsivo e protege dados do cliente', async () => {
  ...
  assert.match(tv, /Alternância inteligente/)
  assert.match(tv, /UPCOMING_NOTICE_MINUTES = 10/)
  assert.match(tv, /AGENDA_ALERT_MS = 45_000/)
  assert.match(tv, /change\.eventType === 'INSERT'/)
  assert.match(tv, /showAgendaTemporarily/)
  assert.match(tv, /aria-hidden=\{showYoutube/)
