...
test('painel do barbeiro isola agenda e venda baixa estoque uma única vez', async () => {
  ...
  try {
    ...
    await client.query("INSERT INTO public.profissionais(id,barbearia_id,nome,ativo,user_id,comissao_percentual,comissao_produtos_percentual) VALUES($1,$2,'Barbeiro Varejo',true,$3,40,12)", [barberId, tenantId, barberUserId])
    ...
    const created = await client.query("SELECT public.produto_catalogo_criar('Pomada teste','POM-1','Finalização',NULL,30,10,5,2,NULL) result")
    ...
    assert.equal(Number(listed.rows[0].comissao_percentual), 12)
    ...
    assert.equal(Number(firstSale.rows[0].result.comissao_valor), 7.2)
    ...
    assert.equal(Number(summary.rows[0].result.comissao_produtos), 7.2)
  } finally {
