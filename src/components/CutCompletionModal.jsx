...
import { Camera, CheckCircle2, ImagePlus, Scissors, Sparkles } from 'lucide-react'
import Button from './ui/Button'
import Input from './ui/Input'
import Label from './ui/Label'
import Modal from './ui/Modal'
import Spinner from './ui/Spinner'
import { concluirAtendimentoComCorte, mensagemErroCorte, obterUrlFotoCorte } from '../lib/clientes/cortes-api'
...
export default function CutCompletionModal({ appointment, open, onClose, onSuccess }) {
  ...
  async function submit(event) {
    ...
    setError('')
    try {
      const result = await concluirAtendimentoComCorte({
  ...
  return (
    <Modal
      ...
      title="Concluir e registrar o corte"
      className="sm:max-w-3xl"
      footer={(
        <>
          <Button variant="secondary" disabled={saving} onClick={onClose}>Voltar</Button>
          <Button type="submit" form="cut-completion-form" loading={saving} disabled={compressing || style.trim().length < 2}><CheckCircle2 size={17} /> Concluir atendimento</Button>
        </>
      )}
    >
