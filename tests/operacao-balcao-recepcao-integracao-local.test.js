...
test('recepção devolve atendimento com histórico e conclui venda avulsa atômica', async () => {
  ...
  try {
    ...
    const sold = await db.query("SELECT public.recepcao_venda_avulsa_concluir($1::jsonb,$2,8,'credito',2,current_date,$3) result", [items, professionalId, saleKey])
    const repeated = await db.query("SELECT public.recepcao_venda_avulsa_concluir($1::jsonb,$2,8,'credito',2,current_date,$3) result", [items, professionalId, saleKey])
    ...
    const receivable = (await db.query('SELECT valor_bruto,taxa,status FROM public.financeiro_contas_receber WHERE venda_balcao_id=$1', [counterSale.id])).rows[0]
    ...
    const directSale = await db.query("SELECT public.recepcao_venda_avulsa_concluir($1::jsonb,NULL,0,'pix',0,current_date,$2) result", [JSON.stringify([{ produto_id: secondProductId, quantidade: 1 }]), crypto.randomUUID()])
